...
**Status:** Revised — round 7 (3 review rounds + architect re-design + 3 comprehensive 6-agent code-grounded audits) incorporated.
...
- `src/lib/portfolio/chart-enrichment.ts` — `seedDisp = firstSliceVal − (lookupCostAtOrBefore(costSeries, chartStart, viewMode) ?? 0)` (gap-from-firstSliceVal #94 seed re-anchor; Task 3.4).
...
booked AND drop a real outflow from the benchmark; reuse the Task-1.3 `quantityDelta` helper) (audit-3 + audit-r5:
