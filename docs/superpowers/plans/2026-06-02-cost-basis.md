...
**Status:** Revised — round 8 (3 review rounds + architect re-design + 3 full 6-agent + 1 targeted code-grounded audits; the last clean) incorporated.
...
- **EXPLANATORY COPY:** every tickbox/button/type-option carries simple, accurate helper text describing what it does and what will happen. The verbatim strings live in `src/lib/cost-basis-copy.ts` (Task 1.2) so they're tested once and reused; components import them.
...
- `src/lib/portfolio/cost-basis.ts` — pure `computeCostBasis(txnsAsc, currentValue, opts?)` engine + `computeAssetPnL(txns, {valueEur, valueUsd})` per-currency wrapper. (The cost time-series `buildCostBasisSeries` lives in `historical-prices-augmentation.ts`, NOT here — see Task 3.4.)
- `src/lib/actions/transactions.ts` — `addTransaction` / `editTransaction` / `markAsYield` / `getAssetTransactions` server actions (ownership-verified).
...
- `src/lib/portfolio/historical-prices-augmentation.ts` — new `buildCostBasisSeries` (over ALL positions, fed cost+delta streams — NOT #94's backdated-only lots; audit-3); split sign reads `split_direction × split_quantity` (C1, ~861-864/899-902).
- `src/lib/actions/backfill.ts` — add `.eq("cashflow_user_set", false)` to the cashflow-backfill WHERE + `backfillSingleRow` (defense: never clobber a user-typed cost; audit-3/r6).
- `src/lib/types.ts` — `AssetRef` discriminated union (T2.4) + `SplitLeg.cost?` (T4.1).
- `src/lib/portfolio/chart-enrichment.ts` — `seedUsd = firstSliceUsd − (lookupCostAtOrBefore(costSeries, chartStart, viewMode) ?? 0)` (gap-from-firstSliceUsd #94 seed re-anchor, USD; Task 3.4).
...
- [ ] **Step 1: Failing test** — call `upsertPosition`, `upsertStockPosition`, and a cash create, each with an override `{ usd, eur }` (+ `cashflowUserSet:true`); assert the logged `activity_log` row carries `cashflow_amount_usd/eur = override` + `cashflow_user_set=true` (NOT `qty × price`). With no override → unchanged (market value, `user_set=false`). Also: (i) **new-Yield** — call with `isYield:true` (no prior row) and assert the inserted row has `is_yield=true` (audit-3 BLOCKER — the only insert path); (ii) **case 16** — pass a single-currency user amount (EUR only) on a given `effective_date` and assert the **other** currency (USD) is stored via FX-at-that-date (`toUsdAndEur`), so both columns persist.
- [ ] **Step 2: Run → FAIL.**
- [ ] **Step 3: Implement** —
  ...
  - `crypto.ts` (`upsertPosition`) + `stocks.ts` (`upsertStockPosition`): accept `opts.cashflowOverride` **and `opts.isYield`**; pass `cashflowOverride` to the `computeActivityFx*` call + pass `cashflow_user_set` **and `is_yield`** to `logActivity` (⚠ audit-3 BLOCKER — the new-Yield row is an INSERT through these primitives, so `is_yield` MUST be settable here, not only via the UPDATE-based editTransaction/markAsYield).
  - `cash-accounts.ts`: add `cashflowOverride` **+ `isYield`** to `CashAccountOpts`; since cash uses `computeFx`/`computeCashflow`, branch to the override there (skip the balance-delta computation); forward `cashflow_user_set` **and `is_yield`** at the cash `logActivity` call-sites — **create + update** (the two that addTransaction's deposit-style yield routes through; `mergeCashAccounts` delegates to update/delete and itself logs nothing — audit-r5).
  - `backfill.ts`: add `.eq("cashflow_user_set", false)` to the cashflow-backfill WHERE (`:42-47`) so the daily cron never overwrites a user-authored cost with market value (audit-r5 — defense even if a user-cost row is briefly `pending`). **Also guard `backfillSingleRow` (`:329-397`, the UI retry path — a second, independent clobber path the primary WHERE doesn't cover; audit-r6):** select `cashflow_user_set` and skip its cashflow write when `true` (the delta write may proceed).
...
- [ ] **Step 1: Failing tests (CORRECTED audit-r5 — `deriveCashFlows` ONLY):** (a) seed a complete cash flow + a yield entry on the same asset; assert `deriveCashFlows` returns the cash flow but **not** the yield row. (b) seed a **backdated** yield row (`effective_date`); assert it is absent from `deriveCashFlows` AND the synthetic flows, **AND that its UNITS still appear in the value/truth line** (`augmentAndExtendSnapshots`) — the key non-regression: yield units are holdings. ⚠ Assert the reconstructed **quantity/value contribution** at a pre-`capture_date` snapshot (not just a nonzero dollar — qty is the property the filter could break; audit-r6).
- [ ] **Step 2: Run → FAIL.**
- [ ] **Step 3: Implement:** add `is_yield` to the `.select(...)` and `.eq("is_yield", false)` in **`deriveCashFlows` ONLY**. ⚠ Do **NOT** filter `fetchHistoricalPriceInputsFor` — its lot stream is the SHARED input to the value line, so a row-level `is_yield` filter would drop yield UNITS from the truth line (audit-r5 BLOCKER). The synthetic reader `buildBenchmarkCashFlows` already excludes yield structurally (it gates on `is_adjustment !== true`; yield is `is_adjustment=false`). Confirm no #94 benchmark OR value-line test regresses (`npm test -- benchmark`).
- [ ] **Step 4: Run → PASS.**
...
**Files:** Modify `crypto-table.tsx` / `stock-table.tsx` / `cash-table.tsx` (action cluster); new read in `src/lib/actions/transactions.ts`; `src/lib/types.ts` (the `AssetRef` discriminated union)

> ⚠ **Engine-input contract (audit-2 + audit-3):** the read feeds the cost engine, so it must be **asset-level,
> NOT per-`entity_id`** — `entity_id` for crypto/stocks is the **per-wallet position id**, so one asset across N
> wallets has N `entity_id`s; grouping by `entity_id` fractures the stream and a wallet↔wallet move stops netting
> to 0 (spurious realized gain). Resolve `assetRef` → asset (`crypto_asset_id`/`stock_asset_id`) → **all of that
> asset's position ids** (RLS-scoped) → `.in("entity_id", positionIds)`. **Cash** is the exception (`entity_id =
> cash_accounts.id`, the account *is* the asset). Keep **`.eq("user_id", user.id)`** (defense-in-depth — positions
> carry no `user_id`), **`.is("undone_at", null)`** (a split marks parent→`undone` right *after* inserting
> children — **sequential, not one transaction**, audit-r5 → **defensively de-dup (audit-r6): build a Set of the
> `split_from_id` values present in the stream and drop any row whose `id` is in it**, so a mid-split-crash
> orphan (a live parent with live children) can't double-count), and **select both `cashflow_amount_*` AND `delta_*`** (C3:
> transfer/adjustment legs carry their value in `delta_*`, not `cashflow_amount_*`).
> ⚠ **Dual-client contract (audit-r5 HIGH):** signature **`getAssetTransactions(supabase, userId, assetRef)`** so the **share page** (non-owner, admin
> client scoped by `owner_id` — `shared-portfolio.ts`) can feed the engine for Task 3.3d's read-only cost P&L;
> mirror `fetchHistoricalPriceInputsFor`'s owner/authed duality. RLS-only leaves the share page with no cost data.
> Define `assetRef` as a typed discriminated union in `types.ts` (`{class:'crypto'|'stock';assetId} | {class:'cash';accountId}`).
...
- [ ] **Step 1: Failing integration tests** — `getAssetTransactions(assetRef)` returns all `activity_log` rows across the asset's wallet positions as **one merged stream** (resolve asset → all position ids → `.in("entity_id", …)`), RLS-scoped + **`.eq("user_id", user.id)`**, **`.is("undone_at", null)`**, **selecting `cashflow_amount_*` + `delta_*`**, **paginated via `fetchAllPaginated`** (GHO can exceed defaults), ordered by `COALESCE(effective_date, created_at)`. Assert: (i) an asset held in **2 wallets** returns a single merged stream and a wallet↔wallet move between them nets to **zero** realized in the engine; (ii) a split parent (now `undone`) is excluded and its live children are included; (iii) **ownership** — a foreign-user `assetRef` returns nothing (cross-user isolation, #97).
...
**Logic sanity (assert) — review-3 + audit-2 + audit-3 corrections:** `addTransaction(type)` maps to the
**override-extended** primitive (Buy/Sell→`upsertPosition` (crypto) / `upsertStockPosition` (stocks) with
`cashflowOverride`; Deposit/Withdrawal→cash action with override; **Yield→a normal qty-up acquisition with the
primitive's new `isYield:true` opt (Task 1.4b), amount left intact, never zeroed**). **`editTransaction` is a
direct `activity_log` UPDATE** — `logActivity` is insert-only, so editing an existing row's
amount/date/`is_yield`/`cashflow_user_set` is an UPDATE (mirror `backdateActivityEntry`/`toggleActivityAdjustment`,
activity-log.ts:380-407), scoped `.eq("id").eq("user_id")` (404 if absent — #97) **and `.is("undone_at", null)`**
(TOCTOU, M4). **Guards:** reject a **transfer leg** (`transfer_group_id != null`), a **split child / undone row**
(`split_from_id != null` or `undone_at != null`), **and a compensation row (`compensates_for != null` — audit-3)**,
with the `COST_COPY.transferLegLocked` / `splitChildLocked` messages (+ a compensation message). **Write amount
AND status — all 8 columns (audit-3 HIGH + audit-r5):** mirror `toggleActivityAdjustment`'s full UPDATE —
`is_adjustment` row → `{delta_*, delta_status:'complete', cashflow_amount_*:null, cashflow_status:null,
cashflow_asset_class:null}`; real-flow row → the inverse **with `cashflow_asset_class` set via
`classifyAssetClass(...)`** (omitting it mis-labels the contribution's class; for a `crypto_position` replicate
the toggle's `isStablecoin(subcategory)` pre-check so stablecoins bucket as cash — audit-r6);
**never both** (`deriveCashFlows` keys on `cashflow_status`, so a stale status re-creates the phantom
contribution). Validate at the boundary: `validateUUID(entryId)` + amount/qty/date (M3). `markAsYield(ids)`
validates each `validateUUID(id)`, flips only **eligible** rows — predicate **`is_adjustment=false AND
transfer_group_id IS NULL AND split_from_id IS NULL AND undone_at IS NULL AND compensates_for IS NULL AND
cashflow_status='complete' AND is_yield=false`** PLUS **`quantityDelta(row) > 0`** (checked in the action after
the fetch — direction isn't pure SQL; **audit-r6 HIGH**: yield is a qty-UP acquisition, so a **Sell/Withdrawal
disposal** (`removed` or qty-down `updated`) must NOT be yield-able — it would zero cost at €0 with no realized
booked AND drop a real outflow from the benchmark; reuse the Task-1.3 `quantityDelta` helper; ⚠ **the markAsYield
+ editTransaction fetches must `select('*')`** to pull the before/after snapshots + `details` the guard reads
(else `quantityDelta` sees `undefined` → returns 0 → silently skips every row) AND `crypto_asset_id` for
editTransaction's `subcategory` lookup — audit-r7) (audit-3 + audit-r5:
`split_from_id` essential — bulk path, a live split child otherwise passes; `cashflow_status='complete'` is the
concrete real-flow test; `is_yield=false` keeps `{updated}` honest) — sets **`is_yield=true` (amount NOT zeroed —
H2 → un-yield is lossless)**, everything else → `skipped`; returns `{updated, skipped}`.

- [ ] **Step 1: Failing tests** — per the sanity list above, incl. `editTransaction` rejecting a transfer leg + a split child + **a compensation row**; the `is_adjustment`→`delta_*` branch asserting `cashflow_amount_*` **AND `cashflow_status`** both stay null **and the real-flow branch sets `cashflow_asset_class`** (audit-r5); `addTransaction(type='yield')` on a NEW asset producing a row with `is_yield=true` (audit-3 BLOCKER); and `markAsYield` **skipping** a transfer leg, an `is_adjustment` correction, a compensation row, a split child, an already-yield row, **and a Sell/Withdrawal disposal (`quantityDelta ≤ 0` — audit-r6)** (→ `skipped`).
...
describe("computeCostBasis — engine cases from spec §10 + invariant", () => {
  ...
  it("5 · yield adds units at cost 0 and lowers avg (engine half; benchmark-exclusion → T2.1)", () => { /* GHO case */ });
  it("6 · yield then sold → realized = full proceeds (cost 0)", () => { /* case 6 */ });
  it("7 · airdrop / new asset as yield → first txn cost 0", () => { /* case 7 */ });
  it("8 · cash yield (bank interest) → units + at cost 0 (engine half; exclusion → T2.1)", () => { /* case 8 */ });
  ...
  it("30 · oversell (disposal exceeds running units) is clamped + fires onAnomaly (H4)", () => {
    const spy = vi.fn();
    // a lone sell row (before=1, after=0 → qtyDelta −1) processed while running units = 0
    // (the supplying buy is missing / backdated later in the sorted stream) → genuine oversell
    const r = computeCostBasis([mk(1,0,150,{sell:true})], 0, { onAnomaly: spy });
    expect(r.costBasis).toBeGreaterThanOrEqual(0);    // clamped: never negative
    expect(r.avgCost).toBeGreaterThanOrEqual(0);
    expect(spy).toHaveBeenCalled();
  });
  it("31a · correction balance-DOWN is off-book → identity does NOT hold across it (H3)", () => { /* case 31 */ });
  it("31b · correction balance-UP → units rise at ZERO cost (identity preserved)", () => { /* case 31 up */ });
  it("INVARIANT (buy/sell/yield only): totalPnL === currentValue + Σproceeds − Σcost (tolerance EPS for cent-rounded inputs)", () => { /* property test, excludes corrections */ });
});
// Cases living in OTHER files (not the pure engine): 10 (double-count) → benchmark/integration; 14 (split math) →
// split-helpers.test; 15 (backdate recompute) → cost-basis-backdate.test; 16 (dual-currency storage) → Task 1.4b; 26 (per-currency divergence) → Task 3.2; 20
...
- [ ] **Step 1-5 (per sub-task):** TDD the per-asset display (avg cost, unrealized, realized, total P&L €+%,
  `changeColor`) — **3.3a calls `computeAssetPnL(txns, {valueEur, valueUsd})` per asset** (the per-currency
  wrapper from Task 3.2); EUR-authoritative headline + USD secondary with `COST_COPY.fxDivergenceTooltip`;
  dashboard Total P&L stat; share-page read-only parity; commit each sub-task. **Manual-NAV display (audit-3):**
  the subscription-cost field rides the **position** write (`upsertStockPosition`), surfaced in the Add-Manual-NAV
  modal's position leg + the shared position editor; the NAV modals stay cost-free (no cost input there).
...
**Design rule (spec §9, re-architected per audit-2 C2):** a scalar seed is **infeasible** — `enrichWithSp500Benchmark` runs **client-side** in a `useMemo`, `chartStart` is **period-dependent** (24H/3D/…/All) and **per-view-mode** (crypto/stocks/cash/total), the engine is per-asset. So build a **server-side, portfolio-wide, per-class running-cost series** and look it up per period/mode at the seed. Corrections/synthetic `is_adjustment` flows stay **market-valued** (the scope bound + the byte-identical guard). **(Amendment 2026-06-04 — overlay uniformity with sells: only BARE corrections stay market-valued; TRANSFER legs (`is_adjustment` + `transfer_group_id`) fold through the engine like sells. Gap/seed unaffected — transfer legs are `cashflow_user_set=false`.)**

- [ ] **Step 3.4a-pre — WIDEN #94's price-fetch for user-costed lots (audit-r5 F2 — the load-bearing fix).** `marketAtChartStart` for a user-costed lot needs cached `historical_prices` + FX, but `ensureHistoricalPricesCached`/`buildHistoricalLots` fetch prices **only for backdated lots** (`:601` drops non-backdated), and a user-costed lot is usually a *non-backdated* normal buy → no cached price → `lotContributionAtDate` returns null → gap silently wrong. Extend the price-fetch to ALSO cover the assets of `cashflow_user_set=true` lots back to `chartStart` (bounded — only assets the user costed).
  - ⚠ **Keep `prices` SEPARATE from the value-line `lots` (audit-r6 HIGH — the one way this breaks the truth line):** the value line is lot-driven and ignores unreferenced price-index entries, so widen the returned `prices` WITHOUT adding the user-costed lots to the returned `lots` (`buildHistoricalLots` keeps `lots` backdated-only). Implement via an explicit `extraPriceCoverage` input to `ensureHistoricalPricesCached`, or an internal `valueLineLots` (returned as `lots`) vs `costedLots` (price-coverage only) split.
  - ⚠ **Defeat the coarse per-`asset_key` cache gate (audit-r6 MEDIUM):** the skip-check isn't per-date-range, so an asset already cached for a *narrower* window won't re-fetch the deeper `[chartStart…]` → null price → silent gap miss (the round-5-class trap); track `MIN(price_date)` per key (`price_date` is already SELECTed at `:523`; re-fetch when chartStart precedes it) or unconditionally fetch `[chartStart…]` for user-costed assets (idempotent upsert via the existing `ignoreDuplicates` constraint).
  - ⚠ **Thread an explicit range, not the shared `rangeStart` scalar (audit-r7 F1):** `ensureHistoricalPricesCached`'s `rangeStart` (`:635`) is one scalar over backdated lots; the widening must supply `rangeStart = chartStart` for costed assets, or widen the shared scalar to `min(rangeStart, chartStart)` (harmless idempotent over-fetch). The fetch is server-side + `cache()`-keyed on `userId` only (no client `chartStart`/`viewMode`), so fetch back to the **All-period start** (the earliest possible chartStart); the per-period seed lookup slices the wide series client-side.
  - Failing tests: (i) a non-backdated user-costed lot has a cached price at chartStart after the widening; (ii) **`getHistoricalPriceInputs(...).lots` is byte-identical pre/post-widening** (only `.prices` grows — the value line MUST NOT change; re-run `historical-prices-augmentation.test.ts` + the augment suite); (iii) an asset with PRE-EXISTING narrower coverage still gets the deeper range.
- [ ] **Step 3.4a — `buildCostBasisSeries` (pure, server-side, over ALL positions — audit-3 BLOCKER).** Create it in `historical-prices-augmentation.ts`. ⚠ It must run over **all** of the user's positions, NOT #94's backdated-only lots. Feed it the **per-asset cost + `delta` streams** + **`cashflow_user_set`** (the gap gate); value each **real** lot at its **cost** (user amount, else market fallback) and each **`is_adjustment`** lot at **market** (`lotContributionAtDate`). **(Amendment 2026-06-04 — overlay uniformity with sells: market valuation applies to BARE corrections only (`is_adjustment` AND `transfer_group_id IS NULL`); TRANSFER legs fold through the engine like sells. Gap/seed unaffected — transfer legs are `cashflow_user_set=false`.)** Apply the **same stablecoin crypto→cash reclassification** the value line uses (`aggregate.ts`/augmentation ~865-887) so per-class cost matches per-class value. **Emit BOTH (audit-r5 F1):** absolute per-class **cost** columns `{ date, cryptoCostUsd, …, …Eur }` (for the Phase-5 overlay) AND explicit per-class **gap** columns `{ cryptoGapUsd, … }` (USD authoritative for the seed — audit-r6) = `Σ(lotContributionAtDate(lot,date) − userCost)` over `cashflow_user_set=true AND NOT is_yield` lots (for the seed — the `NOT is_yield` prevents a user-costed yield row leaking cost the engine zeroes; the gap CANNOT be recovered from an aggregated cost total, so emit it while per-lot data is in scope). Failing unit test: a real backdated lot (cost €5,000) → `cryptoCostUsd` shows €5,000; an `is_adjustment` lot → its **market** value, not 0; a stablecoin lot lands in the **cash** columns; a user-costed lot (cost €5k, market €8k) → `cryptoGapUsd` = €3,000.
- [ ] **Step 3.4b — expose via `getHistoricalBenchmarkExtension`** (`benchmark.ts:145-198`): add the series to its return. ⚠ This is a **third read** (audit-3) — neither "reuse #94's lots" (no cost) nor "superset of `deriveCashFlows`" (selects no `delta_*`, excludes `is_adjustment`). Spell out the SELECT: per-asset rows over **all** positions with `cashflow_amount_*`, `delta_*`, **`cashflow_user_set`** (the gap gate — audit-r5 F3), `is_adjustment`, `is_yield`, `effective_date`/`created_at`, `entity_id`, `details`. (`deriveCashFlows` itself excludes `is_yield` and uses the user amount — Task 2.1.)
- [ ] **Step 3.4c — thread the prop:** `page.tsx:157` + `share/[token]/page.tsx:75` → `PortfolioChart` → `enrichChartData` → `enrichWithSp500Benchmark`. Add `lookupCostAtOrBefore(series, chartStart, viewMode)` that **reads/sums the pre-computed per-class `*GapUsd` columns** (audit-r5 F1 — NOT recomputed from a cost total; the seed is USD-only, so `*GapEur` is reserved for the Phase-5 EUR overlay — audit-r7) for the view-mode (total/investments by summation) — **0** when none are user-costed.
...
- [ ] **Step 3.4e — Write failing seed tests — UNIT, via the public `enrichChartData`:**
  - A backdated lot, cost €5,000, **market €8,000** at chartStart → `costGap = 3,000` → S&P line at chartStart = `firstSliceVal − 3,000` (cost), portfolio line = market — the €3,000 gap survives.
  - A **non-backdated** user-costed lot (cost ≠ market) that predates a short-period `chartStart` → its gap is applied too (validates the 3.4a-pre price widening; tolerate `EPS` on this user-costed branch — reconstructed market may differ from the snapshot by FX/forward-fill ULPs, audit-r5 F4).
  - Control 1: no user cost (gap 0) → `seedDisp == firstSliceVal` **byte-identical** (exact, not EPS).
  - **Control 2 (the real guard — audit-3): a *populated* series** (a backdated lot present, cost == market → gap 0) → **still byte-identical** (proves the gap-subtraction, not an empty-series accident).
  - An `is_adjustment` lot present at chartStart → never user-costed → not in the gap → byte-identical.
- [ ] **Step 3.4f — Implement in USD (audit-r6):** `seedUsd = firstSliceUsd − (lookupCostAtOrBefore(series, chartStart, viewMode) ?? 0)` (the lookup sums the `*GapUsd` columns) → `neededUnits = seedUsd / sp500StartPrice` — seed entirely in USD, sidestepping the dual-EUR FX derivation (`firstSliceUsd` is already available). The seed is a **delta from the real `firstSliceUsd`** (NOT a reconstruction), so a zero gap is byte-identical via the same code path; keep the `unitsAtChartStart` machinery intact. ⚠ **Preserve the empty-slice skip (audit-r7):** when `firstSliceUsd ≤ 0` (the tier-2/3/4 fallback that fixed a prior EUR-seeding bug), gap=0 → seed stays 0 → today's *skip-seeding*; do NOT seed from `firstSliceUsd=0`. **Re-run the ENTIRE existing benchmark + chart-enrichment suite** (`npm test -- benchmark chart-enrichment`) — must be unchanged.
...
- [ ] **Step 1: Failing tests** — splitting `2 BTC €40k` into legs `0.5@2019 €4k · 0.5@2021 €8k · 1.0@2023 €28k`
  creates children whose `cashflow_amount` = the entered per-leg cost; **one `details.split_direction =
  Math.sign(extractQuantity(parent))` is stored on EVERY child** (legs are always positive — `splits.ts` rejects
  `leg.quantity<=0` — so direction is **parent-derived, not per-leg**; `split_quantity` stays POSITIVE; B3/C1) —
  splitting a SELL yields `split_direction = -1` so `quantityDelta` (Task 1.3) keeps it a disposal, not a buy,
  **without double-signing #94's augmentation or showing "−0.5" in the timeline**; **children inherit `is_yield`
  + `cashflow_user_set` from the parent (audit-3 — else a split yield lot silently becomes cost-bearing buys the
  benchmark counts)**; quantities sum to parent; `unsplit` restores; a leg with no cost falls back to
  proportional. **Legacy regression test:** a real #94-shape child (`updated` parent → child `action='updated'`,
  positive `split_quantity`, **no** `split_direction`) defaults to `+1` — exactly the old
  `(action==='removed'?-1:1)` value — so #94 stays byte-identical.
- [ ] **Step 2-4:** add optional `cost` to `SplitLeg` **and to the `split-modal.tsx` `onSplit` callback's
  ...
  use entered cost when present, store one parent-derived `details.split_direction = Math.sign(extractQuantity(parent))`
  on each child (keep `split_quantity` positive), and **copy `is_yield`/`cashflow_user_set` into the child insert
  object** (splits.ts ~174-195). **Then update #94's two augmentation sites** (historical-prices-augmentation.ts
...
- [ ] **Step 1-5:** add a third `<Line>` (faint, dashed) fed by the Phase-3 **`buildCostBasisSeries`** (per-class cost at each date); legend + explainer; respect existing chart modes; TDD render; commit.
...
- **Type consistency:** `TransactionKind`, `quantityDelta`, `classifyTransaction`, `computeCostBasis`, `computeAssetPnL`, `buildCostBasisSeries`, `getAssetTransactions`, `TYPE_GUIDANCE`/`COST_COPY` names are used identically across tasks.
