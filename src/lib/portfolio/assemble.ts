...
export async function assemblePortfolioView(
  ...
  const insights = computeDashboardInsights({
    ...
    sp500Price: (indexPrices["^GSPC"] ?? null)?.price ?? 0,
    sp500Change24h: (indexPrices["^GSPC"] ?? null)?.change24h ?? 0,
    goldPrice: (indexPrices["GC=F"] ?? null)?.price ?? 0,
    goldChange24h: (indexPrices["GC=F"] ?? null)?.change24h ?? 0,
    nasdaqPrice: (indexPrices["^IXIC"] ?? null)?.price ?? 0,
    nasdaqChange24h: (indexPrices["^IXIC"] ?? null)?.change24h ?? 0,
    dowPrice: (indexPrices["^DJI"] ?? null)?.price ?? 0,
    dowChange24h: (indexPrices["^DJI"] ?? null)?.change24h ?? 0,
    eurUsdChange24h: eurUsdData?.change24h ?? 0,
    solPriceUsd: cryptoPrices["solana"]?.usd ?? 0,
    solChange24h: cryptoPrices["solana"]?.usd_24h_change ?? 0,
    stoxx50Price: (indexPrices["^STOXX50E"] ?? null)?.price ?? 0,
    stoxx50Change24h: (indexPrices["^STOXX50E"] ?? null)?.change24h ?? 0,
    silverPrice: (indexPrices["SI=F"] ?? null)?.price ?? 0,
    silverChange24h: (indexPrices["SI=F"] ?? null)?.change24h ?? 0,
    oilPrice: (indexPrices["BZ=F"] ?? null)?.price ?? 0,
    oilChange24h: (indexPrices["BZ=F"] ?? null)?.change24h ?? 0,
    treasury10yPrice: (indexPrices["^TNX"] ?? null)?.price ?? 0,
    treasury10yChange24h: (indexPrices["^TNX"] ?? null)?.change24h ?? 0,
    vixPrice: (indexPrices["^VIX"] ?? null)?.price ?? 0,
    vixChange24h: (indexPrices["^VIX"] ?? null)?.change24h ?? 0,
    dividends,
  });
