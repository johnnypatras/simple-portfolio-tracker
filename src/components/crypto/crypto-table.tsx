...
type CryptoGroupMode = "flat" | "source" | "wallet";

const GROUP_MODE_CYCLE: CryptoGroupMode[] = ["flat", "source", "wallet"];
...
export function CryptoTable({ assets, prices, wallets, primaryCurrency }: CryptoTableProps) {
  ...
  }, [rows, sourceGroups, walletGroups, groupMode, isGrouped, allGroupsExpanded, expanded]);
  ...
  const toggleGroupExpand = useCallback((groupKey: string) => {
    setExpandedGroups((prev) => {
      ...
      if (entries) {
        const assetIds = entries.map((e) => e.row.id);
        setExpanded((prevExp) => {
  ...
  }, [groupMode, sourceGroups, walletGroups]);
