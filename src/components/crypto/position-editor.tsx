...
import type { CryptoAssetWithPositions, Wallet, TransferMode, AcquisitionType, EditorTradePrefill } from "@/lib/types";
...
export function PositionEditor({
  ...
  // ─── Metadata payload shared by every upsert call for a row ─
  function rowInput(walletId: string, quantity: number) {
    const edit = edits[walletId];
    ...
    return {
      ...
      wallet_id: walletId,
      quantity,
      acquisition_method: (edit?.acquisition ?? "bought") as AcquisitionType,
      // `apy` explicit 0 is VALID (no yield); only blank/NaN defers (undefined
      // → partialUpdate leaves the stored value untouched).
      apy: Number.isNaN(apy) ? undefined : apy,
      network: edit?.network?.trim() || null,
    };
  }

  function metadataDirty(walletId: string): boolean {
    const edit = edits[walletId];
    const orig = originals[walletId];
    if (!orig) return false; // new rows: routed answers persist no metadata (spec §8.12)
    return (
      edit.acquisition !== orig.acquisition ||
      edit.apy !== orig.apy ||
      edit.network !== orig.network
  ...
  /** Zero-delta save (original quantity) — books no benchmark flow. */
  async function persistMetadataIfDirty(walletId: string): Promise<void> {
    if (!metadataDirty(walletId)) return;
    const existingPosition = asset.positions.find((p) => p.wallet_id === walletId);
    if (!existingPosition) return;
    const priceData = prices?.[asset.coingecko_id];
    await upsertPosition(rowInput(walletId, existingPosition.quantity), {
      currentPriceUsd: priceData?.usd,
      currentPriceEur: priceData?.eur,
    });
  }

  /** Save dispatcher: metadata-only saves go straight through; any quantity
   *  delta opens the intent step (C3) — the write happens from its answer. */
  function handleSave(walletId: string) {
    setError(null);
    const edit = edits[walletId];
    ...
    if (delta === 0) {
      void saveMetadataOnly(walletId);
      return;
    }
    setIntentFor({ walletId, delta, positionId: existingPosition?.id ?? null, deleteRow: false });
  }

  async function saveMetadataOnly(walletId: string) {
  ...
  async function handleIntentBuy(cost: { amount: number; currency: string } | null) {
    ...
    onTrade("buy", {
      ...
      walletId,
      walletOption: { id: walletId, name: wallet?.name ?? "Unknown" },
    });
  }
  ...
  async function handleIntentYield(date: string | null) {
    ...
    try {
      ...
      await addTransaction(
        { class: "crypto", assetId: asset.id },
        { type: "yield", quantity: Math.abs(delta), effectiveDate: date ?? undefined, walletId },
      );
  ...
  async function handleIntentCosmetic(date: string | null) {
    ...
    try {
      if (deleteRow && existingPosition) {
        await deletePosition(existingPosition.id, {
          isAdjustment: true,
        ...
        const qty = parseFloat(edit?.quantity ?? "0");
        await upsertPosition(rowInput(walletId, qty), {
          isAdjustment: true,
          ...(omitPrice ? {} : { currentPriceUsd: priceData?.usd, currentPriceEur: priceData?.eur }),
          ...(date ? { effectiveDate: date } : {}),
        });
      ...
      setAdjOverrides((prev) => ({ ...prev, [walletId]: true }));
      setJustSavedId(walletId);
   ...
   *  The cosmetic answer routes to deletePosition via intentFor.deleteRow. */
  function handleDelete(positionId: string, walletId: string) {
    const existingPosition = asset.positions.find((p) => p.id === positionId);
    const qty = existingPosition?.quantity ?? 0;
    if (qty <= 0) return;
    setError(null);
    setIntentFor({ walletId, delta: -qty, positionId, deleteRow: true });
  }
  ...
  function handleAddWallet() {
    ...
    setEdits((prev) => ({
      ...prev,
      [addingWallet]: { quantity: "0", acquisition: "bought", apy: "0", network: "" },
    }));
    setAddingWallet("");
  }
  ...
  return (
    ...
    >
      {intentFor ? (
        <EditorIntentStep
          key={intentFor.walletId}
          ticker={asset.ticker}
          delta={intentFor.delta}
          approxValueEur={approxDeltaValueEur({
            kind: "crypto",
            absDelta: Math.abs(intentFor.delta),
            priceEur: prices?.[asset.coingecko_id]?.eur,
          })}
          lastWasTransfer={
            asset.positions.find((p) => p.wallet_id === intentFor.walletId)?.last_was_transfer ?? false
          }
          ...
          onOpenTransfer={() => {
            const wId = intentFor.walletId;
            setIntentFor(null);
            setMoveSourceWalletId(wId);
